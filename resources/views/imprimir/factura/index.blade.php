                        ...
                        <table class="datos_cliente">
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        <p>{{date('M j, Y', strtotime($nota->created_at))}}</p>
                                    @endif
                                </td>
                            </tr>
                            <tr>
            ...
            <tbody id="detalle_totales">
                <tr>
                    ...
                    <td colspan="2" width="450px">
                        <p><strong>SON:</strong> {{($tipo_documento == 1)?convertir($factura->valor):convertir($nota->valor)}} PESOS - COP</p>
                    </td>
                    <td colspan="2" class="textright" width="150px"><span><strong>Subtotal.</strong></span></td>
                    <td class="textright" ><span><strong>$ {{($tipo_documento == 1)?number_format($factura->valor):number_format($nota->valor)}}</strong></span></td>
                </tr>
                <tr>
                    <td rowspan="3" colspan="2" >
                        <p><strong>Observaciones:</strong> {{($tipo_documento == 1)?$factura->observaciones:$nota->concepto}}</p>
                        <p>
                            @if($new_pag > 0)
                    ...
                    <td class="textright"><span><strong>$ {{number_format($factura->iva)}}</strong></span></td>
                </tr>
                <tr>
                    <td colspan="2" class="textright" ><span><strong>RETEFUENTE</strong></span></td>
                    <td class="textright">
                        <span><strong>
                            @php
                                $retencion = 0.00;
                                if($factura->clientes->aplicaretencion == 1 && $factura->valor > 980000){
                                    $retencion = ($factura->calidad_retenedor * $factura->valor) / 100;
                                }
                            @endphp
                                $ {{number_format($retencion)}}</strong></span>
                    </td>
                ...
                <tr>
                    <td colspan="6"><p class="nota">Autorizacion de facturacion DIAN No. 18764044495310 Fecha: 2023/02/15</p></td>
                </tr>
            </tbody>
        </table>
    </div>
    ...
    <div id="page_pdf" style="page-break-before:always;">
        @for($i=0; $i < 3; $i++)
            <div class="rotulo">
                ...
                <h2>{{$mundep_cliente->municipios->mcpio}} - {{$mundep_cliente->departamentos->nombre_dpto}}</h2>
                <hr>
                <h2>ENVIA: <strong>ANDRES VELEZ</strong></h2>
                <h2>TEL: 3145920820</h2>
            </div>
        @endfor
    </div>
    <footer>
